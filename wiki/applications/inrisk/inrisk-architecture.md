...
application: inrisk
state: current
sources: [20260422-meeting-transcript-session-1, 20260422-meeting-transcript-session-2, 20260513-inrisk-integration-with-party-mdm-follow-up, 20260514-inrisk-high-level-refinement, 20260519-mdm-implementation-strategy]
source_count: 5
status: draft
---
...
- **UI stack (correction, 2026-05-19)**: InRisk is **primarily Tailwind**, **not** Chakra. Earlier wiki passes (and the [[inrisk-cuts-over-before-high-volume]] ADR as originally written) leaned on "Chakra 2" as the baseline; [[rory-beattie]] corrected this in [[sources/20260519-mdm-implementation-strategy]]: _"that's a bit of a lie. They have some Chakra V2 components, but it's like a drop-down list here, and a button there. They're primarily on Tailwind and a few other bits and pieces. So the closer you can get to raw CSS when you build it, the better."_ Implication on the Phase-1 widget: the new party-search widget for InRisk is built in **raw React + close-to-raw CSS**, reusing existing InRisk components where possible (refines [[inrisk-cuts-over-before-high-volume]] Part B).
- **Widget dispatch code (Phase-1 change)**: InRisk has **dispatch logic in its own codebase** that decides which of [[party-application]]'s widgets to call — _"in-risk have something in their code that determines which of our widgets they call. Yes, they pass parameters to us, but they specifically request it, so we'd need to change that on InRisk's side"_ ([[alex-sillars]], [[sources/20260519-mdm-implementation-strategy]]). The Phase-1 widget rebuild requires a **corresponding InRisk-side dispatch-code change** to call the new single-component-three-variants widget instead of today's separate widgets. **Owner**: [[billy-calladine]] (formalised 2026-05-19 on [[party-rearch-ownership-matrix]] action #38).
...
- **Phase-1 target**: bespoke, parity-only widget — **raw React + close-to-raw CSS**, reusing existing InRisk components where possible (refined 2026-05-19, was "design-system-agnostic library" until then). **One component with three parameterised variants** — _parties_, _brokers_, _other_ — each hitting a different OpenSearch endpoint and rendering different cards, hosted inside a **shell container** (Rory's framing: _"a shell component which is the container — the drawer that manifests in the in-risk world — and then within that, you can manifest either one variant, another variant, or the third variant of the widget components within that one shell"_). **SDK-style** integration — instantiate the MDM SDK, pass methods / hooks to the widget components — but matched to InRisk's current auth / RBAC / session flow and look-and-feel. **Drop-in replacement now, enhancement later** (rolled back from Joe's original "modern and different" vision at the 2026-05-14 HL): _"my original scope was don't affect in-risk too much, so I just went crazy and was like, no, I want it to be really modern and different, but, yeah, we can roll it back a bit"_ ([[joe-worsfold]]). The widget is already published in its Chakra-3-with-design-system form (for PCT); the raw-React-on-InRisk variant is the rebuild work. **Owner**: [[billy-calladine]] (formalised 2026-05-19). See [[inrisk-cuts-over-before-high-volume]].
...
- **No design-system surface; primarily Tailwind** (correction 2026-05-19) — InRisk has _some_ Chakra-2 components but not as a design system; the dominant UI stack is **Tailwind + bits and pieces**. Per [[inrisk-cuts-over-before-high-volume]], InRisk does **not** absorb Chakra 3 + design-system migration under the September gate; the new widget is built in **raw React + close-to-raw CSS**, reusing InRisk components where possible.
- **Widget dispatch code change required on InRisk side** — the existing "which widget to call" dispatch logic in InRisk's codebase needs the corresponding update when the new single-component-three-variants widget lands. [[billy-calladine]] owns both halves (formalised 2026-05-19; see [[party-rearch-ownership-matrix]] action #38).
...
- [[sources/20260519-mdm-implementation-strategy]] — **Tailwind correction** (InRisk is primarily Tailwind, not Chakra); widget rebuild = raw React + close-to-raw CSS (single component, three parameterised variants in a shell); InRisk-side dispatch-code change explicitly required; [[billy-calladine]] formalised as owner of both halves

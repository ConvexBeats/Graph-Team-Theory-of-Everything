...
- **Data distribution** — who reads Party events: [[data-universe]] (owned by [[analytics-team]]), party-versioning in [[inrisk]] + [[artificial]], **[[knowledge-graph]]** (Graph-team-owned read-replica that powers InRisk search — corrected back into this bucket 2026-05-26 per the [[sources/20260519-mdm-implementation-strategy]] ingest; see contradiction note on [[contract-buckets]]), Launchpad, party reporting, EVA, broker dashboards.
...
- **Sanctions orchestration lives in Boomi** (2026-05-13): Boomi consumes every party-change event, calls InRisk one submission at a time to reconstruct context (no batch endpoint), maintains its own cache + idempotency, calls [[ntt]] for the actual check. Group consensus: wrong place. See [[sanctions-processing]] · [[open-questions#OQ-032]]. Phase 1 leaves this flow unchanged. From the Party-side view: Joe describes this as _"this weird triangle of data going backwards and forwards"_ ([[sources/20260519-mdm-implementation-strategy]]).
- **Boomi → Party write-back contract is undocumented** (surfaced 2026-05-27): when NTT returns, Boomi writes the sanctions decision onto the Party record (_"we update our party with that"_, [[alex-sillars]], [[sources/20260422-meeting-transcript-session-2]]) — but the specific API endpoint / event shape and which ID Boomi uses as the lookup key (legacy Graph party ID vs UUID vs other) aren't captured in any source. Phase-1 cutover transparency under [[strangle-the-graph-via-proxy-events]] + [[uuid-system-id-with-display-id]] depends on knowing this. See [[open-questions#OQ-045]].
- **[[knowledge-graph]] reads from InRisk + Party** (correction 2026-05-26): KG is a separate Graph-team-owned graph DB that read-replicates InRisk + Party data and powers [[inrisk]]'s search functionality. Feed mechanism pending; **not** internal to [[party-application]] as the 2026-04-22 lint pass had concluded. See [[open-questions#OQ-010-R]].
- **Three known event consumers** (per [[joe-worsfold]], [[sources/20260519-mdm-implementation-strategy]]): DU, HV (Phase-1 onward via [[boomi]]), and Boomi (for sanctions). DU doesn't care about submission/requirement IDs.
...
Key Phase-1 changes:
- **DU sees no change** on cutover — proxy events in Graph shape. Joe's articulation 2026-05-19: _"we have to come up with this proxy solution"_ because restructuring downstream consumers (DU + HV + Boomi) to absorb a different event shape in Phase-1 time is unrealistic. See [[strangle-the-graph-via-proxy-events]] refinement (2026-05-19, afternoon).
- **Proxy-event-with-InRisk-data design fork is open**: MDM needs to enrich the proxy event with InRisk-side context (submissions, requirements) at emission time so Boomi keeps working. Two candidate sources: (a) an InRisk endpoint (Joe's preference; clean ownership), (b) a pull from [[knowledge-graph]] (parked over consistency concerns by Joe + Alex). Gates the proxy adapter. [[open-questions#OQ-041]].
...
- **Two widget component libraries** — Joe publishes a **bespoke InRisk widget on raw React + close-to-raw CSS** (refined 2026-05-19; InRisk is primarily Tailwind not Chakra) alongside the Chakra-3 + design-system widget for [[party-curation-tool]] / [[dataops-team]]. HV doesn't use a widget. Closes [[open-questions#OQ-005]]. **Drop-in-replacement / parity-not-enhancement posture for the InRisk widget** (refined 2026-05-14) — same auth / RBAC / session / look-and-feel / filter parameters (incl. **TOBA status**); UX improvements deferred. **Widget structure** (refined 2026-05-19): one component, three parameterised variants (parties / brokers / other), each hitting a different OpenSearch endpoint, hosted in a shell container. **Owner**: [[billy-calladine]] — formalised 2026-05-19 for both the widget rebuild AND the InRisk-side dispatch-code change (the existing "which widget to call" logic).
...
| **Proxy-event InRisk-data lookup (gates the proxy adapter)** | [[graph-team]] ([[joe-worsfold]]) | [[prebind-team]] ([[john-trahearn]] / [[kris-mokrzycki]]) for Option A; [[graph-team]] for Option B | 1 | **New 2026-05-19**. MDM emits proxy events that need submission/requirement context; two options: (A) call an InRisk endpoint `client_id → {submissions, requirements}` (Joe's preference; real-time consistency) — InRisk-side endpoint work; (B) pull from [[knowledge-graph]] — Graph-team-side read-path work, eventually consistent (parked over consistency concerns by Joe + Alex). Decision pending sanctions session with [[simon-hulbert]]. [[open-questions#OQ-041]] |
| **Integration-env JumpCloud SAML Cognito access** | platform / Cognito-owning team | [[graph-team]] ([[joe-worsfold]]) + integration-env consumers ([[artificial]] etc.) | 1 | **New 2026-05-19**. MDM is stood up on the integration env but no one can exercise it because JumpCloud SAML Cognito access is broken. Unblocking task. [[open-questions#OQ-042]] |
| **Dynamo migration tooling — Phase-1 scope** | [[graph-team]] ([[joe-worsfold]]) | [[graph-team]] (internal) + future [[dataops-team]] (self-service) | 1 (scope TBC) | **New 2026-05-19**. Dynamo doesn't accept Postgres-style scripts; Joe wants a self-service tool eventually. Phase-1-vs-fast-follow-vs-ad-hoc open. [[open-questions#OQ-043]] |
| **PCT new-UI dataops parity confirmation** | [[graph-team]] ([[alex-sillars]] · [[tomas-sivo]]) | [[dataops-team]] · [[hugh-lobban]] (sponsor) | 1 | **New 2026-05-19**. New PCT shape is revisions + filters with mark-for-review / approve / request-changes baked in (vs current table + tabs + Jira). Walkthrough / user-test needed before cutover. [[open-questions#OQ-044]] |
| **InRisk-side widget dispatch-code change** | [[graph-team]] ([[billy-calladine]]) | [[prebind-team]] | 1 | **New 2026-05-19**. InRisk has dispatch logic in its own code that picks which Party widget to call; the new single-component-three-variants widget requires the corresponding InRisk-side update. Billy owns both halves (widget rebuild + dispatch change). [[party-rearch-ownership-matrix]] action #38 |
| **Boomi ↔ Party write-back contract + identifier usage** | [[graph-team]] ([[joe-worsfold]]) ↔ [[high-volume-team]] ([[srini]]) — sanctions-side | escalation: [[rory-beattie]] | 1 | **New 2026-05-27** (wiki query). What API/event does [[boomi]] use to write [[ntt]] sanctions results back into [[party-application]] today, and which identifier does it use as the lookup key? Closest claim is _"we update our party with that"_ ([[alex-sillars]], [[sources/20260422-meeting-transcript-session-2]]); precise contract isn't in any source. Phase-1 cutover transparency under [[strangle-the-graph-via-proxy-events]] + [[uuid-system-id-with-display-id]] depends on knowing this. Symmetric to OQ-041 (which covers Boomi's _inbound_ proxy-event shape) — this covers Boomi's _outbound_ write into Party. [[open-questions#OQ-045]] |
...
- 2026-05-26 — [[sources/20260519-mdm-implementation-strategy]] — **Knowledge Graph correction**: data-distribution bucket now lists [[knowledge-graph]] explicitly (corrected back in after the 2026-04-22 lint pass had removed it on a wrong-classification basis); current-state notable-quirks list updated with KG read-replica framing and the "three known event consumers" enumeration (DU + HV + Boomi). Phase-1 diagram annotations: proxy-event-with-InRisk-data design fork (Option A InRisk endpoint vs Option B KG pull) added as gating for the proxy adapter; two-libraries widget call sharpened to "raw React + close-to-raw CSS for InRisk" (Tailwind correction); widget structure clarified as one component, three parameterised variants; Billy formalised on the widget rebuild + InRisk-side dispatch-code change. **Five new cross-cutting rows**: proxy-event InRisk-data lookup; integration-env JumpCloud SAML Cognito access; Dynamo migration tooling Phase-1 scope; PCT new-UI dataops parity confirmation; InRisk-side widget dispatch-code change.
- 2026-05-27 — wiki query (Boomi ↔ Party sanctions write-back) surfaced a gap: the API contract Boomi uses to write the NTT result back into Party — and the identifier it uses — is not documented in any source we hold. Current-state notable-quirks list extended with a `Boomi → Party write-back contract is undocumented` bullet; one new cross-cutting row added (Boomi ↔ Party write-back contract + identifier usage). Tracked at [[open-questions#OQ-045]].
